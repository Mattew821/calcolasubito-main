...
export default function AreaRettangoloPage() {
  ...
  const [result, setResult] = useState<ReturnType<typeof calculateRectangleAreaDetailed> | null>(null)
  ...
  const onCalculate = (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const detailed = calculateRectangleAreaDetailed(Number(base), Number(height), unit)
      setResult(detailed)
    } catch (err) {
      setResult(null)
      ...
      {result !== null && (
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-4" role="status" aria-live="polite">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-sm text-gray-600">Perimetro ({unit})</p>
              <p className="text-2xl font-bold text-blue-700">{formatNumber(result.perimeterInInputUnit)}</p>
            </div>
          </div>
