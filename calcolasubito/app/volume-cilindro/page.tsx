...
import { calculateCylinderVolume } from '@/lib/calculations'

function formatNumber(value: number): string {
  return value.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 4 })
}

export default function VolumeCilindroPage() {
  const [radius, setRadius] = useState('5')
  const [height, setHeight] = useState('10')
  const [error, setError] = useState<string | null>(null)
  const [result, setResult] = useState<ReturnType<typeof calculateCylinderVolume> | null>(null)
  ...
  const onCalculate = (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const output = calculateCylinderVolume({ radius: Number(radius), height: Number(height) })
      setResult(output)
    } catch (err) {
      setResult(null)
  ...
  return (
    <Calculator title="Volume Cilindro" description="Calcola volume, area laterale e totale di un cilindro.">
      ...
      {result !== null && (
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3" role="status" aria-live="polite">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-gray-600">Area Laterale</p>
              <p className="text-lg font-semibold text-gray-800">{formatNumber(result.lateralArea)} m²</p>
            </div>
            <div>
